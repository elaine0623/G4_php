...
try {
    ...
    $checkAccount = "SELECT * FROM admin WHERE am_account = :am_account AND am_password = :am_password";
    $stmt = $pdo->prepare($checkAccount);
    $stmt->bindValue(':am_account', $data['acc']);
    $stmt->bindValue(':am_password', $data['psw']);
    $stmt->execute();
    if ($stmt->rowCount() > 0) {
        $userData = $stmt->fetch(PDO::FETCH_ASSOC);
        //判斷是否停權
        $permission = $userData['am_status'];
        if ($permission == 0) {
